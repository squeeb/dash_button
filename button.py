...
class IftttWebHook:
    ...
    def send_event(self, options = {}):
        logging.debug('in %s::%s', self.__class__.__name__,'send_event')
...
class DoorBell(object):
    ...
    def chime(self):
        logging.debug('in %s::%s', self.__class__.__name__,'chime')
...
class ButtonEvent(object):
    ...
    def get_button_pressed(self, hwsrc):
        logging.debug('in %s::%s', self.__class__.__name__,'get_button_pressed')
